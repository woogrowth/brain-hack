  ...
  return h('div',{className:'overlay',onClick:()=>setState({modal:null})},
    h('div',{className:'modal-box',style:{maxWidth:'400px',width:'100%'},onClick:e=>e.stopPropagation()},
      h('div',{className:'mhdr'},
        h('span',{className:'mtag'},'// SYS_AUTH'),
        h('button',{className:'xbtn',onClick:()=>setState({modal:null})},'✕')
      ),
      h('div',{style:{padding:'24px'}},
        field('IDENTIFIER','',h('input',{className:'inp',placeholder:'아이디',value:state.loginForm.username,onInput:e=>setQ({loginForm:{...state.loginForm,username:e.target.value}})})),
        field('PASSKEY','',h('div',{style:{position:'relative'}},
        ...
        h('button',{className:'cpbtn primary',style:{width:'100%',padding:'12px'},onClick:handle},'JACK IN →'),
        h('button',{className:'cpbtn ghost',style:{width:'100%',padding:'10px',marginTop:'8px'},onClick:()=>setState({modal:'signup',signupStep:0,signupData:{job:'',country:'KR',username:'',password:'',pwConfirm:'',nickname:'',agreePrivacy:false,agreeThird:false,agreePush:false}})},'계정 없음 → 회원가입'),
        null
      )
    )
  );
...
function renderSignupModal() {
  ...
  const finishSignup = ()=>{
    ...
    const charEmoji = job?.[charKey] || job?.char || '🧑';
    const user = {
      username:d.username, password:d.password, nickname:d.nickname,
      job:d.job, country:d.country, gender,
  ...
  if(s===0) content = h('div',{},
    ...
    field('닉네임 (2자 이상)',errs.nickname||'',h('input',{className:'inp',placeholder:'표시될 닉네임',value:d.nickname,onInput:e=>{state.signupData={...state.signupData,nickname:e.target.value}}})),
    field('IDENTIFIER (아이디 6자 이상)',errs.username||'',h('input',{className:'inp',placeholder:'brain_user01',value:d.username,onInput:e=>{state.signupData={...state.signupData,username:e.target.value}}})),
    field('PASSKEY (8자+숫자)',errs.password||'',h('div',{style:{position:'relative'}},
      h('input',{className:'inp',style:{paddingRight:'40px'},type:state.showPwSignup?'text':'password',placeholder:'••••••••',value:d.password,onInput:e=>{state.signupData={...state.signupData,password:e.target.value}}}),
      h('button',{style:{position:'absolute',right:'10px',top:'50%',transform:'translateY(-50%)',background:'transparent',border:'none',cursor:'pointer',fontSize:'16px',color:'var(--dim)',lineHeight:'1'},
        onClick:()=>setState({showPwSignup:!state.showPwSignup})},state.showPwSignup?'🙈':'👁️')
    )),
    field('PASSKEY CONFIRM',errs.pwConfirm||'',h('div',{style:{position:'relative'}},
      h('input',{className:'inp',style:{paddingRight:'40px'},type:state.showPwSignup2?'text':'password',placeholder:'••••••••',value:d.pwConfirm,onInput:e=>{state.signupData={...state.signupData,pwConfirm:e.target.value}}}),
      h('button',{style:{position:'absolute',right:'10px',top:'50%',transform:'translateY(-50%)',background:'transparent',border:'none',cursor:'pointer',fontSize:'16px',color:'var(--dim)',lineHeight:'1'},
    ...
    h('div',{style:{fontFamily:'var(--display)',fontSize:'15px',color:'var(--cyan)',marginBottom:'8px'}},'PUSH NOTIFICATION'),
  ...
  return h('div',{className:'overlay'},
    h('div',{className:'modal-box',style:{maxWidth:'500px',width:'100%'}},
      h('div',{className:'mhdr'},
        ...
        h('span',{className:'mtag'},'// REGISTER'),
