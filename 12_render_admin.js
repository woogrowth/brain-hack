function renderSysStats() {
  ...
  // Recent 7 days activity (fake + attendance based)
...
function renderAppealsLog() {
  ...
  return h('div',{style:{padding:'4px 0'}},
    h('div',{style:{display:'flex',alignItems:'center',gap:'10px',marginBottom:'14px'}},
      h('div',{style:{fontFamily:'var(--mono)',fontSize:'11px',color:'var(--cyan)',letterSpacing:'2px'}},'// APPEALS_LOG — AUDIT TERMINAL'),
      h('span',{style:{fontFamily:'var(--mono)',fontSize:'9px',color:'var(--dim)'}},logs.length+' entries'),
      h('button',{className:'cpbtn ghost sm',style:{marginLeft:'auto',fontSize:'10px'},onClick:downloadTxt},'⬇ .txt 다운로드')
    ),
    // Terminal log
    h('div',{style:{background:'#010503',border:'1px solid #00ff4133',padding:'12px 14px',height:'280px',overflowY:'auto',
      ...
      h('div',{style:{color:'#00ff41',marginBottom:'8px',borderBottom:'1px solid #00ff4122',paddingBottom:'6px'}},
        '> BRAINHACK APPEALS LOG v2.0 | '+new Date().toLocaleString('ko-KR')),
      logs.length===0 ? h('div',{style:{color:'#1a4030',marginTop:'20px',textAlign:'center'}},'[ NO LOG ENTRIES ]') :
      [...logs].reverse().map(l=>{
        const col = typeColors[l.type]||'var(--dim)';
        return h('div',{style:{marginBottom:'8px',borderLeft:'2px solid '+col+'44',paddingLeft:'8px'}},
          h('div',{style:{color:'#3a6050',fontSize:'9px'}},new Date(l.timestamp).toLocaleString('ko-KR')+' | by '+l.by),
...
function renderAdmin() {
  ...
  if(state.adminTab==='problems') tabContent = h('div',{},
    h('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px'}},
      h('span',{className:'sec-title'},'// PROBLEM DATABASE — '+probs.length+' ENTRIES'),
      h('button',{className:'cpbtn ghost sm',onClick:()=>{ ls.set('cp_problems',[...probs,{id:Date.now(),category:'기타',difficulty:3,question:'새 문제',hint:'',answer:'정답',explanation:'',timeLimit:60,xp:20,coins:10}]); render(); }},'+ 수동 추가')
    ),
    ...probs.map(p=>h('div',{className:'prow'},
      ...
      h('button',{className:'cpbtn danger sm',onClick:()=>delProblem(p.id)},'DEL')
    ))
  );
    ...
    ...users.map(u=>h('div',{style:{background:'var(--panel)',border:'1px solid '+(u.banned?'#ff003c33':'var(--border)'),
      ...
      h('select',{className:'inp',style:{width:'auto',padding:'4px 8px',fontSize:'11px'},
        onChange:e=>{ if(e.target.value){banUser(u.username,e.target.value==='permanent'?'permanent':Number(e.target.value));e.target.value='';}}},
      ...
      u.banned?h('button',{className:'cpbtn ghost sm',onClick:()=>unbanUser(u.username)},'해제'):null
    ))
  );
