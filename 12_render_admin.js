            ...
            h('div',{style:{marginBottom:'20px'}},
              ...
              h('div',{style:{display:'flex',gap:'8px'}},
                ...
                h('button',{className:'cpbtn primary sm',onClick:()=>{
                  ...
                  const next = users.map(x=>x.username===mu.username?{...x,coins:(x.coins||0)+val}:x);
                  ls.set('cp_users',next); setState({adminMemberManage:mu.username});
            ...
            h('div',{style:{marginBottom:'20px'}},
              ...
              h('div',{style:{display:'flex',gap:'8px'}},
                ...
                h('button',{className:'cpbtn secondary sm',onClick:()=>{
                  ...
                  const next = users.map(x=>x.username===mu.username?{...x,xp:(x.xp||0)+val}:x);
                  ls.set('cp_users',next); setState({adminMemberManage:mu.username});
              ...
              h('div',{style:{display:'flex',gap:'8px'}},
                ...
                h('button',{className:'cpbtn ghost sm',onClick:()=>{
                  ...
                  if(!mu.inventory?.includes(itemId)) {
                    const next = users.map(x=>x.username===mu.username?{...x,inventory:[...(x.inventory||[]),itemId]}:x);
                    ls.set('cp_users',next); setState({adminMemberManage:mu.username});
              ...
              h('div',{style:{display:'flex',gap:'6px',flexWrap:'wrap'}},
                ...(mu.inventory||[]).map(id=>{
                  ...
                  return h('button',{className:'nbtn',style:{fontSize:'10px',borderColor:'var(--pink)',color:'var(--pink)'},
                    onClick:()=>{
                      const next = users.map(x=>x.username===mu.username?{...x,inventory:(x.inventory||[]).filter(y=>y!==id),equipped:Object.fromEntries(Object.entries(x.equipped||{}).map(([k,v])=>[k,v===id?'':v]))}:x);
                      ls.set('cp_users',next); setState({adminMemberManage:mu.username});
