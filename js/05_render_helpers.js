...
function h(tag, attrs, ...children) {
  ...
  let pendingSelectValue = undefined; // select는 option 추가 후 value 설정해야 함
  if(attrs) {
    for(const [k,v] of Object.entries(attrs)) {
      ...
      else if(k==='value' && tag==='select') { pendingSelectValue=v; } // option 추가 후 처리
  ...
  // select: option이 모두 추가된 후 value 설정해야 반영됨
  if(pendingSelectValue !== undefined) el.value = pendingSelectValue;
  return el;
}
