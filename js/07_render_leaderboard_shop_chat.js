  ...
  return h('div',{className:'lb-wrap'},
    ...
    ...sorted.map((u,i)=>{
      ...
      return h('div',{className:'lbrow'+(u.username===state.currentUser?.username?' me':'')},
        ...
        h('span',{style:{fontFamily:'var(--mono)',fontSize:'13px',color:'var(--green)',marginLeft:'10px'}},
          state.lbTab==='score'?(u.score||0)+' pts':state.lbTab==='xp'?(u.xp||0).toLocaleString()+' XP':(u.solved||[]).length+'문제')
      );
    })
  );
}
    ...
    return h('div',{style:{padding:'28px 0'}},
      ...
      h('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:'12px'}},
        ...filtered.map(item=>{
          ...
          const buy = ()=>{
            if((state.currentUser.coins||0)<item.price){showToast('코인이 부족합니다!','error');return;}
            if(isOwned&&!isPotion){showToast('이미 보유 중입니다.','info');return;}
            const newCoins = (state.currentUser.coins||0)-item.price;
            updateUser({...state.currentUser, coins:newCoins, inventory:[...owned,item.id]});
            addActivityLog('shop_buy',{item:item.id,price:item.price});
          ...
          return h('div',{style:{background:'var(--panel)',border:'1px solid '+(isEquipped?'var(--cyan)':isOwned?'#00ff9f44':'var(--border)'),
            ...
            h('div',{style:{fontFamily:'var(--mono)',fontSize:'11px',color:'var(--yellow)',marginBottom:'12px'}},'💰 '+item.price),
            isOwned&&!isPotion?
        ...
        ...EMOJI_SHOP.map((emoji,i)=>{
          ...
          return h('div',{
            ...
            onClick:()=>{
              ...
              if(!isOwned){
                if((state.currentUser.coins||0)<10000){showToast('코인이 부족합니다! (10,000코인 필요)','error');return;}
                updateUser({...state.currentUser,coins:(state.currentUser.coins||0)-10000,inventory:[...owned,eid],char:emoji});
                addActivityLog('emoji_buy',{emoji});
                showToast(emoji+' 이모지 구매 및 적용 완료!','success');
              } else {
