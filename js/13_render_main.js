...
function renderSettingsModal() {
  ...
  if(tab === 'display') {
    tabContent = h('div',{},
      h('div',{className:'slabel',style:{marginBottom:'10px'}},'다크/라이트 모드'),
      h('div',{style:{display:'flex',gap:'8px',marginBottom:'18px'}},
        h('button',{className:'cpbtn '+(state.darkMode?'secondary':'ghost')+' sm',onClick:()=>{
          const dm = !state.darkMode;
          setState({darkMode:dm});
          ls.set('bh_darkMode', dm);
          applyTheme();
    ...
    tab==='search' ? h('div',{},
      h('div',{style:{display:'flex',gap:'8px',marginBottom:'14px'}},
        h('input',{className:'inp',style:{flex:'1'},placeholder:'닉네임 또는 아이디 검색 (2자 이상)',
          value:state.socialSearch||'',onInput:e=>setQ({socialSearch:e.target.value})}),
